...
int posaljiPoruku(int sock, int vrstaPoruke, const char *poruka){
        int duljinaPoruke=strlen(poruka); //duljina poruke koju saljemo

        int poslano, poslanoZadnje;
        //trebamo duljinaPoruke pretvoriti u mrezni oblik pri slanju
    ...
    //poslali koliko ce biti duljina, sada gledamo za nasu vrstu poruke
...
int primiPoruku(int sock, int *vrstaPoruke, char **poruka){
    ...
    if(primljenoZadnje!=sizeof(duljinaPoruke_n))
        return NIJEOK;
    ...
    *poruka=(char*)malloc((duljinaPoruke+1)*sizeof(char));
    ...
    while(primljeno!=duljinaPoruke){
        ...
        if(primljenoZadnje==-1||primljenoZadnje==0)
            return NIJEOK;
